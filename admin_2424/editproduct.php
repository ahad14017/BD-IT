...
if (!isset($_SESSION['admin_login'])) {
	$user = "";
	include_once("login.php?ono=".$epid."");
	exit();
}
else {
	if (isset($_REQUEST['epid'])) {
	
		$epid = $mysqli -> real_escape_string($_REQUEST['epid']);
	}else {
...
$getposts = $mysqli -> query("SELECT * FROM products WHERE id ='$epid'") or die(mysql_error());
...
if (isset($_POST['updatepro'])) {
	...
	if($result = $mysqli -> query("UPDATE products SET pName='$_POST[pname]',price='$_POST[price]',description='$_POST[descri]',available='$_POST[available]',category='$_POST[category]',type='$_POST[type]',item='$_POST[item]',pCode='$_POST[code]' WHERE id='$epid'")){
		include_once("editproduct.php?epid=".$epid."");
...
//triming name
	$getposts1 = $mysqli -> query("SELECT pid FROM orders WHERE pid='$epid'") or die(mysql_error());
					if ($ttl = mysqli_num_rows($getposts1)) {
						$error_message = "You can not delete this product.<br>Someone ordered this.";
					}
					else {
						if($mysqli -> query("DELETE FROM products WHERE id='$epid'")){
								...
								<div class="signup_form">
									...
									<div>
										...
										font-style: italic;margin-bottom: 3px;margin-top: 0px;padding: 14px;line-height: 25px;border-radius: 4px;border: 1px solid #169E8F;color: #169E8F;margin-left: 0;width: 300px;background-color: transparent;" class="">
												...
												<option value="PC_ACCESSORIES">PC_ACCESSORIES</option>
												<option value="MOBILE_ACCESSORIES">MOBILE_ACCESSORIES</option>
												<option value="MUSIC_SYSTEMS">MUSIC_SYSTEMS</option>
												<option value="TV_&_ANDROID_TV_BOX">TV_&_ANDROID_TV_BOX</option>
												<option value="HAIR_TRIMMER_&_DRYER">HAIR_TRIMMER_&_DRYER</option>
												<option value="WIFI_ROUTER">WIFI_ROUTER</option>
