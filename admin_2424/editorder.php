...
if (!isset($_SESSION['admin_login'])) {
	$user = "";
	include_once('login.php?ono=".$eoid.""');
...
//triming name
	try {
		...
		$get_order_info = $result1 -> fetch_assoc();
			...
			$order_status = $get_order_info['dstatus'];
		...
		else if ($eodstatus == "Cancel" and $order_status!= "Cancel"){
		...
		$get_order_info = $result1 -> fetch_assoc();
		    ...
		    // DB PRODUCT QUANTITY // 
                		    $query = $mysqli -> query("SELECT * FROM products WHERE id ='$eopid'");
                ...
                //echo $product_quantity;
                    ...
                    $product_quantity = $product_quantity+$dquantity;
                    
                    
                $query = $mysqli -> query("UPDATE products SET available='$product_quantity' WHERE id = '$eopid'");
                	
                
		    
		    
		}
		else if ($eodstatus == "Yes" and $order_status == "Cancel"){
		...
		$get_order_info = $result1 -> fetch_assoc();
		    ...
		    // DB PRODUCT QUANTITY // 
				...
				if($mysqli -> query("UPDATE orders SET dstatus='$eodstatus', ddate='$ddate', quantity='$dquantity' WHERE id='$eoid'")){
					//success message
				include_once('editorder.php?eoid='.$eoid.'');
