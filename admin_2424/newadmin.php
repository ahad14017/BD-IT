...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		$check = 0;
		$e_check = $mysqli -> query("SELECT email FROM `admin` WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >4 ) {
						...
						$result = $mysqli -> query("INSERT INTO admin (firstName,lastName,email,mobile,address,password,type,confirmCode) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[signupaddress]','$_POST[password]','$_POST[admintype]','$confirmCode')");
